...
public class LowpassFilters implements PlugInFilter {

    private int threshold, order;
    private int M, N, size, w, h, m, b, salida;
    ...
    boolean showDialog(ImageProcessor ip) {
        ...
        threshold = 20;
        order = 1;
        String[] choices = {"Ideal", "Butterworth", "Gaussian", "Trapezoide"};
        ...
        //int fc = 20;
        if (filter.equals("Trapezoide")) {
            //gd2.addNumericField("Fc", fc, 0);
            gd2.addNumericField("F0", f0, 0);
        }
    ...
    public ByteProcessor Ideal() {
        ...
        for (int radius = 0; radius < threshold; radius++) {
            for (double counter = 0; counter < 10; counter = counter + 0.1) {
        ...
        ip2.insert(ip, w, h);
        if (displayFilter) {
            new ImagePlus("Ideal filter", ip2.duplicate()).show();
        }
        return ip2;
    }
    // creates a Trapezoide lowpass filter
    public ByteProcessor Trapezoide() {
        ByteProcessor ip = new ByteProcessor(M, N);
        int xcenter = (M / 2);
        int ycenter = (N / 2);
  
        for (int radius = 0; radius < f0; radius++) {
            for (double counter = 0; counter < 10; counter = counter + 0.001) {
                ...
                m = -255/(f0-threshold);
                b=-m*f0;
                int value = (m*radius)+b;
                ip.putPixel((int) x, (int) y, value);
                }
        ...
        ip2.insert(ip, w, h);
        if (displayFilter) {
            new ImagePlus("Trapezoide", ip2.duplicate()).show();
        }
        return ip2;
    }
